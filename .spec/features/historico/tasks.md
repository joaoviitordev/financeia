...
- Modelo: claude-sonnet-5
- Esforço: alto
- Notas: depende de T-015. No `App.tsx` a sheet passa a renderizar a lista no lugar do estado vazio embutido, e abrir uma simulação navega para `/resultado/<id>` e fecha a sheet (AC-030). A `HistoryPage` é a mesma lista em página cheia, na rota `/historico` que hoje é placeholder (AC-036). Apagar a simulação que está aberta leva para `/` (ASM-017, AC-035) — quem decide isso é quem tem a rota em mãos, não a lista. O teste do AC-031 prova o que o cache comprou: abrir pelo histórico uma simulação com diagnóstico guardado não dispara nenhuma chamada (`fetch` dublado, contagem zero).
