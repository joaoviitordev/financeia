#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano historico` em 2026-08-22 23:11
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo historico
set -u
set -o pipefail

RUN_ID='financeia-historico-mt4ztbj9'
FEATURE='historico'
BASE_BRANCH='spec/historico'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/historico/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/financeia-historico-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/financeia-historico"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/historico/executar-tarefas.sh --faixa $1"
...
executar_seq_T_014() {
  info 'sequencial T-014 — Excluir e limpar no armazenamento'
  if rodar_tarefa seq 'T-014' 'Você executa UMA tarefa da feature "historico" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/historico/spec.md, .spec/features/historico/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-014 — "Excluir e limpar no armazenamento"
  critérios/refs: AC-033 (Excluir remove só a simulação escolhida, para sempre), AC-034 (Apagar tudo limpa o histórico inteiro, com a mesma cerimônia)
  arquivos permitidos (e seus testes): src/features/simulations/storage.ts, src/features/simulations/storage.test.ts
  mensagem de commit: "T-014 historico: Excluir e limpar no armazenamento"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-014 historico: Excluir e limpar no armazenamento (auto-commit do plano)'
    fi
  ...
  amarelo "  reexecute só ela: bash .spec/features/historico/executar-tarefas.sh --seq T-014"
  FALHAS="$FALHAS T-014"
  return 1
}
...
executar_seq_T_015() {
  info 'sequencial T-015 — A lista do histórico'
  if rodar_tarefa seq 'T-015' 'Você executa UMA tarefa da feature "historico" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/historico/spec.md, .spec/features/historico/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-015 — "A lista do histórico"
  critérios/refs: AC-027 (A lista mostra as simulações, da mais recente para a mais antiga), AC-028 (A lista diz quais já têm diagnóstico), AC-029 (Sem nada guardado, o histórico explica o que vai aparecer ali), AC-032 (Excluir pede confirmação antes)
  arquivos permitidos (e seus testes): src/features/simulations/HistoryList.tsx, src/features/simulations/HistoryList.test.tsx
  mensagem de commit: "T-015 historico: A lista do histórico"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' high >> "$LOG_DIR/seq.log" 2>&1; then
    # commit de segurança se o agente esqueceu (rastreabilidade > perfeição)
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-015 historico: A lista do histórico (auto-commit do plano)'
    fi
  ...
  amarelo "  reexecute só ela: bash .spec/features/historico/executar-tarefas.sh --seq T-015"
  FALHAS="$FALHAS T-015"
  return 1
}
...
executar_seq_T_016() {
  info 'sequencial T-016 — O histórico na sheet e no endereço próprio'
  if rodar_tarefa seq 'T-016' 'Você executa UMA tarefa da feature "historico" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/historico/spec.md, .spec/features/historico/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-016 — "O histórico na sheet e no endereço próprio"
  critérios/refs: AC-030 (Abrir uma simulação leva ao resultado dela), AC-031 (Reabrir pelo histórico não custa uma nova geração), AC-035 (Apagar a simulação aberta devolve a pessoa ao início), AC-036 (`/historico` mostra a mesma lista em página cheia)
  arquivos permitidos (e seus testes): src/App.tsx, src/router.tsx, src/routes/HistoryPage.tsx, src/routes/HistoryPage.test.tsx, src/features/simulations/historico-navegacao.test.tsx
  mensagem de commit: "T-016 historico: O histórico na sheet e no endereço próprio"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' high >> "$LOG_DIR/seq.log" 2>&1; then
    # commit de segurança se o agente esqueceu (rastreabilidade > perfeição)
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-016 historico: O histórico na sheet e no endereço próprio (auto-commit do plano)'
    fi
  ...
  amarelo "  reexecute só ela: bash .spec/features/historico/executar-tarefas.sh --seq T-016"
  FALHAS="$FALHAS T-016"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/historico/executar-tarefas.sh --gate"
      exit 0
    fi
