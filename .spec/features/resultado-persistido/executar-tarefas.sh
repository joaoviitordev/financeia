#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano resultado-persistido` em 2026-08-22 16:19
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo resultado-persistido
set -u
set -o pipefail

RUN_ID='financeia-resultado-persistido-mt4l3dyw'
FEATURE='resultado-persistido'
BASE_BRANCH='spec/resultado-persistido'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/resultado-persistido/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/financeia-resultado-persistido-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/financeia-resultado-persistido"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/resultado-persistido/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/resultado-persistido-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-001' 'Você executa UMA tarefa da feature "resultado-persistido" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/resultado-persistido/spec.md, .spec/features/resultado-persistido/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-001 — "Rotas e layout raiz"
  critérios/refs: AC-001 (O formulário abre na raiz), AC-002 (Recomeçar pelo cabeçalho limpa as respostas)
  arquivos permitidos (e seus testes): src/router.tsx, src/routes/SimulationPage.tsx, src/App.tsx, src/main.tsx, src/App.test.tsx, src/routes/SimulationPage.test.tsx
  mensagem de commit: "T-001 resultado-persistido: Rotas e layout raiz"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/resultado-persistido-faixa-1' "$WT" "$st" || return 1
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/resultado-persistido-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-002' 'Você executa UMA tarefa da feature "resultado-persistido" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/resultado-persistido/spec.md, .spec/features/resultado-persistido/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-002 — "Perguntas de dívidas e prazo, com o cálculo"
  critérios/refs: AC-003 (O fluxo passa a ter sete perguntas), AC-004 (Não ter dívida é uma resposta válida), AC-005 (O prazo aceita de 1 a 120 meses), AC-006 (As dívidas reduzem a sobra mensal)
  arquivos permitidos (e seus testes): src/features/onboarding/questions.ts, src/features/onboarding/fields/types.ts, src/features/onboarding/fields/registry.ts, src/features/onboarding/answers-to-plan.ts, src/features/onboarding/goals.ts, src/components/ui/Field.tsx, src/features/onboarding/goals.test.ts, src/features/onboarding/Onboarding.test.tsx, src/features/onboarding/dividas-e-prazo.test.tsx
  mensagem de commit: "T-002 resultado-persistido: Perguntas de dívidas e prazo, com o cálculo"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' high
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/resultado-persistido-faixa-2' "$WT" "$st" || return 1
...
executar_faixa_3() {
  ...
  preparar_worktree 'faixa-3' 'spec/resultado-persistido-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-003' 'Você executa UMA tarefa da feature "resultado-persistido" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/resultado-persistido/spec.md, .spec/features/resultado-persistido/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-003 — "Armazenamento das simulações"
  critérios/refs: AC-007 (Guardar e reler devolve as mesmas respostas), AC-008 (Cada simulação tem identificador próprio), AC-009 (Armazenamento corrompido não derruba o app)
  arquivos permitidos (e seus testes): src/features/simulations/storage.ts, src/features/simulations/storage.test.ts
  mensagem de commit: "T-003 resultado-persistido: Armazenamento das simulações"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' high
  ...
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/resultado-persistido-faixa-3' "$WT" "$st" || return 1
  marcar_concluidas T-003
  return 0
}

# ── sequencial T-004 (fora da seleção do usuário) ──
executar_seq_T_004() {
  info 'sequencial T-004 — Página de resultado'
  if rodar_tarefa seq 'T-004' 'Você executa UMA tarefa da feature "resultado-persistido" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/resultado-persistido/spec.md, .spec/features/resultado-persistido/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-004 — "Página de resultado"
  critérios/refs: AC-011 (O resultado mostra os números da simulação), AC-012 (Endereço de simulação inexistente é explicado)
  arquivos permitidos (e seus testes): src/routes/ResultPage.tsx, src/features/simulations/SimulationCards.tsx, src/routes/ResultPage.test.tsx
  mensagem de commit: "T-004 resultado-persistido: Página de resultado"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' high >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-004 resultado-persistido: Página de resultado (auto-commit do plano)'
    fi
  ...
  amarelo "  reexecute só ela: bash .spec/features/resultado-persistido/executar-tarefas.sh --seq T-004"
  FALHAS="$FALHAS T-004"
  return 1
}

# ── sequencial T-005 (fora da seleção do usuário) ──
executar_seq_T_005() {
  info 'sequencial T-005 — Conclusão guarda a simulação e navega'
  if rodar_tarefa seq 'T-005' 'Você executa UMA tarefa da feature "resultado-persistido" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/resultado-persistido/spec.md, .spec/features/resultado-persistido/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-005 — "Conclusão guarda a simulação e navega"
  critérios/refs: AC-010 (Concluir leva ao resultado da simulação criada)
  arquivos permitidos (e seus testes): src/features/onboarding/useOnboarding.ts, src/routes/SimulationPage.tsx, src/App.tsx, src/features/simulations/conclusao.test.tsx
  mensagem de commit: "T-005 resultado-persistido: Conclusão guarda a simulação e navega"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium >> "$LOG_DIR/seq.log" 2>&1; then
    # commit de segurança se o agente esqueceu (rastreabilidade > perfeição)
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-005 resultado-persistido: Conclusão guarda a simulação e navega (auto-commit do plano)'
    fi
  ...
  amarelo "  reexecute só ela: bash .spec/features/resultado-persistido/executar-tarefas.sh --seq T-005"
  FALHAS="$FALHAS T-005"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/resultado-persistido/executar-tarefas.sh --gate"
      exit 0
    fi
