#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano chat-educador` em 2026-08-23 12:08
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo chat-educador
set -u
set -o pipefail

RUN_ID='financeia-chat-educador-mt5rk1is'
FEATURE='chat-educador'
BASE_BRANCH='spec/chat-educador'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/chat-educador/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/financeia-chat-educador-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/financeia-chat-educador"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/chat-educador/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/chat-educador-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-017' 'Você executa UMA tarefa da feature "chat-educador" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/chat-educador/spec.md, .spec/features/chat-educador/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-017 — "A conversa no armazenamento"
  critérios/refs: AC-045 (A conversa sobrevive ao recarregar), AC-046 (Mudar uma resposta descarta a conversa junto com o diagnóstico)
  arquivos permitidos (e seus testes): src/features/insights/chat-types.ts, src/features/insights/chat-types.test.ts, src/features/simulations/storage.ts, src/features/simulations/storage.test.ts
  mensagem de commit: "T-017 chat-educador: A conversa no armazenamento"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/chat-educador-faixa-1' "$WT" "$st" || return 1
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/chat-educador-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-018' 'Você executa UMA tarefa da feature "chat-educador" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/chat-educador/spec.md, .spec/features/chat-educador/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-018 — "O pedido de acompanhamento"
  critérios/refs: AC-040 (O pedido carrega a simulação, o diagnóstico e a conversa até aqui), AC-041 (A conversa enviada tem teto)
  arquivos permitidos (e seus testes): src/features/insights/chat.ts, src/features/insights/chat.test.ts
  mensagem de commit: "T-018 chat-educador: O pedido de acompanhamento"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' high
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/chat-educador-faixa-2' "$WT" "$st" || return 1
...
executar_seq_T_019() {
  info 'sequencial T-019 — O painel da conversa'
  if rodar_tarefa seq 'T-019' 'Você executa UMA tarefa da feature "chat-educador" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/chat-educador/spec.md, .spec/features/chat-educador/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-019 — "O painel da conversa"
  critérios/refs: AC-039 (Perguntar devolve uma resposta na conversa), AC-042 (Enter envia, Shift+Enter quebra linha), AC-043 (Não dá para enviar vazio nem duas vezes), AC-044 (A conversa rola até a mensagem nova), AC-047 (Enquanto a resposta não chega, a pergunta mostra que ela está vindo), AC-048 (A pergunta que falhou é dita pelo nome e pode ser reenviada)
  arquivos permitidos (e seus testes): src/features/insights/InsightChat.tsx, src/features/insights/InsightChat.test.tsx
  mensagem de commit: "T-019 chat-educador: O painel da conversa"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' high >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-019 chat-educador: O painel da conversa (auto-commit do plano)'
    fi
  ...
  amarelo "  reexecute só ela: bash .spec/features/chat-educador/executar-tarefas.sh --seq T-019"
  FALHAS="$FALHAS T-019"
  return 1
}
...
executar_seq_T_020() {
  info 'sequencial T-020 — A conversa na página de resultado'
  if rodar_tarefa seq 'T-020' 'Você executa UMA tarefa da feature "chat-educador" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/chat-educador/spec.md, .spec/features/chat-educador/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-020 — "A conversa na página de resultado"
  critérios/refs: AC-038 (A conversa só abre quando há diagnóstico), AC-045 (A conversa sobrevive ao recarregar), AC-046 (Mudar uma resposta descarta a conversa junto com o diagnóstico)
  arquivos permitidos (e seus testes): src/routes/ResultPage.tsx, src/routes/ResultPage.test.tsx, src/features/insights/conversa-persistida.test.tsx
  mensagem de commit: "T-020 chat-educador: A conversa na página de resultado"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' high >> "$LOG_DIR/seq.log" 2>&1; then
    # commit de segurança se o agente esqueceu (rastreabilidade > perfeição)
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-020 chat-educador: A conversa na página de resultado (auto-commit do plano)'
    fi
  ...
  amarelo "  reexecute só ela: bash .spec/features/chat-educador/executar-tarefas.sh --seq T-020"
  FALHAS="$FALHAS T-020"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/chat-educador/executar-tarefas.sh --gate"
      exit 0
    fi
